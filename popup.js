...
function displayRoomInfo() {
  ...
  $('#4floor').change(function() {
    $('.4').toggle('slow', function() {
      saveFilter();
    });
  });
  $('#5floor').change(function() {
    $('.5').toggle('slow', function() {
      saveFilter();
    });
  });
  $('#6floor').change(function() {
    $('.6').toggle('slow', function() {
      saveFilter();
    });
  });
  // get the last setting for filters, default to showing all
  chrome.storage.sync.get({
    ...
    function (items) {
      if (items.show_fourth_floor === false) {
        $('#4floor').change();
        $('#4floor').prop('checked', false);
      }
      if (items.show_fifth_floor === false) {
        $('#5floor').change();
        $('#5floor').prop('checked', false);
      }
      if (items.show_sixth_floor === false) {
        $('#6floor').change();
        $('#6floor').prop('checked', false);
      }
  });
}
